#!/usr/bin/env python3
# server.py – שילוב: pynput לעכבר, keyboard לקיצורים, pyautogui במיוחד ל־CTRL+C
...
def press_combo(keys):
    try:
        combo = '+'.join(keys)
        if combo == 'ctrl+c':
            print("✔ Sending CTRL+C using pyautogui")
...
def main():
    ...
    print(f"✅ Server ready on UDP {COMMAND_PORT}/{DISCOVERY_PORT} using keyboard + pyautogui for CTRL+C")
    try:
